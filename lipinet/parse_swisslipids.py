...
def parse_swisslipids_data(verbose=False, force_download=False, use_cache=False):
    ...
    df_swisslipids = get_prior_knowledge(
        "swisslipids",
        verbose=verbose,
    ...
    df_swisslipids_edges = df_swisslipids_edges[
    ...
    df_swisslipids_edges["target_layer"] = df_swisslipids_edges["target_layer"].map(
        lambda x: "swisslipids"
        if x == "Lipid class*"
        else f"sl_{str(x).replace(' ', '').strip('*').lower()}",
    )

    # For rows where both source and target layers are 'swisslipids', swap the columns so that the parent points to the children
    condition = (df_swisslipids_edges["source_layer"] == "swisslipids") & (
        df_swisslipids_edges["target_layer"] == "swisslipids"
    )
    df_swisslipids_edges.loc[
        condition,
        ["source_layer", "source_id", "target_layer", "target_id"],
    ] = df_swisslipids_edges.loc[
        condition,
        ["target_layer", "target_id", "source_layer", "source_id"],
    ].values
    ...
    mask_slash = df_swisslipids_edges["target_id"].str.contains(
        "/",
        regex=False,
        na=False,
    ) & df_swisslipids_edges["target_layer"].str.contains("sl_components", regex=False, na=False)
    ...
    df_swisslipids_edges["interlayer"] = (
        df_swisslipids_edges["source_layer"] != df_swisslipids_edges["target_layer"]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Run in verbose mode (maximal output from print statements)",
    )
    parser.add_argument(
        "-f",
        "--force-download",
        action="store_true",
        help="Force resource download, even if file present locally already (WARNING: overwrites local copy)",
    )
    ...
    _ = parse_swisslipids_data(verbose=verbose, force_download=force_download)
    if verbose:
