...
def build_rhea_ec_edges_and_nodes(df_ec: pd.DataFrame):
    ...
    df["subclass_node"] = "EC:" + df["Main_Class"].astype(str) + "." + df["Subclass"].astype(str)
    df["subsubclass_node"] = (
        "EC:"
        + df["Main_Class"].astype(str)
        + "."
    ...
    df_main = df[["main_node"]].drop_duplicates().rename(columns={"main_node": "node_id"})
    df_main["ec_level"] = "main_class"

    df_sub = df[["subclass_node"]].drop_duplicates().rename(columns={"subclass_node": "node_id"})
    ...
    df_full = df[["EC_number"]].drop_duplicates().rename(columns={"EC_number": "node_id"})
...
def parse_rhea_data(verbose: bool = False, use_cache: bool = False, force_download: bool = False):
    ...
    df_nodes_reaction = (
        df_rhea[
            [
                ...
                "ChEBI name",
                "EC number",
                "Enzymes",  # note: dropped 'Participant identifier' and 'Enzyme class' bc not part of REST API (seems to be post-processed on client side), not immediately useful, could add downstream
                "Gene Ontology",
    ...
    # ChEBI nodes
    df_nodes_chebi = (
        df_rhea_exploded[["ChEBI identifier", "ChEBI name"]]
...
def main():
    ...
    parser.add_argument(
        "--use-cache",
        action="store_true",
        help="Load/save processed nodes & edges cache",
    )
    parser.add_argument(
        "--force-download",
        action="store_true",
        help="Force fresh raw download and rebuild",
    )
