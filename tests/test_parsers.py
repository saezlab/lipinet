...
def test_parse_swisslipids_data_basic(monkeypatch):
    ...
    assert {"source_layer", "source_id", "target_layer", "target_id", "interlayer"}.issubset(edges.columns)
...
def test_process_ec_numbers_and_edges_nodes():
    ...
    assert set(["Main_Class", "Subclass", "Subsubclass", "Serial_Number", "EC_number"]).issubset(df_ec.columns)
    edges, nodes = pr.build_rhea_ec_edges_and_nodes(df_ec)
    assert {"source_id", "target_id", "source_layer", "target_layer", "interlayer"}.issubset(edges.columns)
...
def test_parse_rhea_fallback_and_cache(monkeypatch):
    ...
    monkeypatch.setattr(pr, "get_prior_knowledge", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("boom")))

    # Monkeypatch pandas.read_csv for the fallback path
    monkeypatch.setattr(pr.pd, "read_csv", lambda *a, **k: pd.DataFrame({
        "Reaction identifier": ["R1"],
        "Equation": ["A=B"],
        "ChEBI identifier": ["CHEBI:1;CHEBI:2"],
        "ChEBI name": ["A;B"],
